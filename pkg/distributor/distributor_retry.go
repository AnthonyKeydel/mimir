...
func (cfg *RetryConfig) RegisterFlags(f *flag.FlagSet) {
	...
	f.IntVar(&cfg.Strategy, "distributor.retry.strategy", 0, "Retry strategies for handling 429/5xx requests. Strategy 0 involves retrying after a duration within the range [1, retry.Base]. Strategy 1 entails retrying after a duration within the range [1, retry.Base * retry-Attempts]. Strategy 2, on the other hand, prescribes retrying after a duration within the range [1, retry.Base * 2^(retryAttempts-1)].")
}
